...
class MainWindow(QDialog): # класс окна, тут описан его функционал
    ...
    def loadImage(self): # загрузить картинку
        ...
        if(False): # если надо выводить по отдельности, то делаем своё окно
    ...
    def applyNoise(self): # применение шума
        global IMAGE_GLOBAL
        IMAGE_GLOBAL = functions.noise(IMAGE_GLOBAL, self.sliderNoise.value())
        if(False): 
    ...
    def filterImage(self): # фильтр
        global IMAGE_GLOBAL
        IMAGE_GLOBAL = functions.denoise(IMAGE_GLOBAL, self.sliderFilter.value(), self.spinBox1.value(), self.spinBox2.value())
        if(False): 
