    ...
    # Функция для выполнения в отдельном потоке
        def run_operation():
            ...
            if res is not None:
                ...
                result_pixmap = result_pixmap.scaled(300, 200)
                self.label_result.setPixmap(result_pixmap)
                self.progressBar.setHidden(True)  # Скрываем прогресс-бар после завершения операции

        # Создаем и запускаем поток для выполнения операции
        operation_thread = threading.Thread(target=run_operation)
        operation_thread.start()

        # Показываем прогресс-бар и запускаем анимацию
